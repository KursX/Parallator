        ...
        cell.setOnMouseClicked(t -> {
            ...
            if (t.getButton() == MouseButton.PRIMARY) {
                ...
                edit.setOnAction(event -> {
                    ...
                    try {
                        FXMLLoader loader = new FXMLLoader(getClass().getResource("layouts/edit_dialog.fxml"));
                ...
                for (int i = 0; i < parts.size(); i++) {
                    if (parts.get(i).trim().isEmpty()) parts.remove(i);
                }

                for (int index = 1; index < parts.size(); index++) {
                    position += parts.get(index - 1).length();
                    dividers.add(data.substring(position, data.indexOf(parts.get(index))));
                    position++;
                }
