...
public class MainController implements Initializable {
    ...
    public void statistics() {
        new Thread(() -> {
            ...
            for (File file1 : ChapterCellFactory.getFilesList(file, "1.txt")) {
                String en = Helper.getTextFromFile(file1, config.enc1()).trim();
                enLength += en.length();
            ...
            Platform.runLater(() -> {
                MainController.this.statistics.setText(
                        String.format("Исходник Количество символов: %1$d Параграфоф: %2$d Слов: %3$d Качество: %4$.2f%%",
                                enL, p, w, quality[0] * 100 / quality[1] ));
            });
        }).start();
    }
